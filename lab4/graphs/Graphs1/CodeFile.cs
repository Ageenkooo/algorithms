...
namespace Graphs1
{
    ...
    class DrawGraph
    {
            ...
            for (int i = 0; i < E.Count; i++)
            {
                ...
                else
                {
                    ...
                    for (int j = 0; j < W.Count; j++)
                    {
                        ...
                        {
                            gr.DrawString(((W[j].value)).ToString(), fo, br, point);
                        }
                    }
                }
            }
        ...
        public void drawALLGraphWithAreas(List<Vertex> V, List<Edge> E, List<Weight> W, string[] used)
        {
            for (int i = 0; i < E.Count; i++)
            {
                if (E[i].v1 == E[i].v2)
                ...
                else
                {
                    gr.DrawLine(darkGoldPen, V[E[i].v1].x, V[E[i].v1].y, V[E[i].v2].x, V[E[i].v2].y);
                    point = new PointF((V[E[i].v1].x + V[E[i].v2].x) / 2, (V[E[i].v1].y + V[E[i].v2].y) / 2);
                    gr.DrawString((used[E[i].v1]).ToString(), fo, br, point);
                }
            }
            for (int i = 0; i < V.Count; i++)
            {
